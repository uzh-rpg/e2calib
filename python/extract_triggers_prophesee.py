...
def get_reconstruction_timestamps(time: np.ndarray, pol: np.ndarray, extract_ts: str = "rising", time_offset_us: int=0):
    assert 0 <= pol.max() <= 1
    assert np.all(np.abs(np.diff(pol)) == 1), 'polarity must alternate from trigger to trigger'

    timestamps = None
    if extract_ts == "middle":
    ...
    elif extract_ts == "rising":
        timestamps = time[pol==1]
    elif extract_ts == "falling":
        timestamps = time[pol==0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_timestamp', choices=["rising", "middle", "falling"], default="rising",
            help='Whether to take the timestamp of the rising, falling, or the middle tiemstamp.')
    ...
    reconstruction_ts = get_reconstruction_timestamps(triggers['t'], triggers['p'], args.use_timestamp, args.time_offset_us)
